...
class CmcHtmlTextHelper {
     ...
     */
    //based on stage_modules/utility/fns.news_events.php chopString()
    public static function HtmlBlogSummary ($strHtml, $maxChars, $stripTags=false, $strClose=" ...") {
        
        if ($stripTags) {
            $strHtml = strip_tags($strHtml);
            //remove everything in square brackets
            $strHtml = preg_replace("/\[[^]]+\]/","",$strHtml);
        }
        //Debug::show($strHtml);
    ...
    public static function NoBlogHtmlChop ($strHtml, $maxChars,  $strClose="") {
        return self::HtmlBlogSummary($strHtml, $maxChars, true, $strClose);
    }
